...
import type { CourseMetricItem } from './courseOperationViewModel';

interface CourseMetricCardProps {
  item: CourseMetricItem;
}

const valueToneClass = (tone?: CourseMetricItem['tone']): string => {
  if (tone === 'rose') return 'text-[#8B4A42]';
  if (tone === 'amber') return 'text-[#8A6A3A]';
  return 'text-[#222622]';
};

const hintToneClass = (tone?: CourseMetricItem['tone']): string => {
  if (tone === 'rose') return 'text-[#9A6B63]';
  if (tone === 'amber') return 'text-[#8A7355]';
  return 'text-[#70776F]';
};

const CourseMetricCard: React.FC<CourseMetricCardProps> = ({ item }) => (
